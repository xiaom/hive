...
ENV HIVE_VERSION 0.13.1
WORKDIR $HIVE_HOME
ADD packaging/target/apache-hive-$HIVE_VERSION-bin/apache-hive-$HIVE_VERSION-bin $HIVE_HOME
...
RUN sed -i 's/^\(bind-address\s.*\)/# \1/' /etc/mysql/my.cnf && \
  ...
  echo "bin/schematool -dbType mysql -initSchemaTo 0.13.0" >> /tmp/config && \
